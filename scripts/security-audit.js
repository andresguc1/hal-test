...
async function runSecurityAudit() {
  ...
  try {
    ...
    const highCount = Object.values(vulns).filter(
  ...
  try {
    ...
    if (results.outdated.length > 0) {
      console.log(
        `⚠️  Found ${results.outdated.length} outdated production packages`,
      );
      results.outdated.slice(0, 10).forEach((pkg) => {
        console.log(
          `   ${pkg.name}: ${pkg.current} -> ${pkg.latest} (wanted: ${pkg.wanted})`,
        );
      });
  ...
  try {
    ...
    const issues = results.licenses.filter((l) =>
      problematicLicenses.some((pl) => l.license?.includes(pl)),
    );
  ...
  try {
    ...
    console.log(
      "✅ Basic secret scan completed (consider using truffleHog for production)",
    );
  } catch (error) {
  ...
  if (!hasHighVulns && !hasOutdated) {
    ...
    if (hasHighVulns) {
      console.log(
        `❌ ${results.vulnerabilities.length} high/critical vulnerabilities`,
      );
    }
    if (hasOutdated) {
