...
async function generateSBOM(packageDir, outputFile, options = {}) {
  ...
  try {
    ...
    console.error(
      `❌ Failed to generate SBOM for ${packageDir}:`,
      error.message,
    );
    return false;
  }
...
async function generateAllSBOMs() {
  ...
  results.forEach((r) => {
    console.log(
      `  ${r.success ? "✅" : "❌"} ${r.name}: ${r.success ? r.outputFile : "FAILED"}`,
    );
  });
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (command === "all") {
    ...
    console.log("Usage:");
    console.log(
      "  node generate-sbom.js all                    - Generate SBOMs for all artifacts",
    );
    console.log(
      "  node generate-sbom.js single <dir> <output>  - Generate SBOM for single package",
    );
    process.exit(1);
  }
}
