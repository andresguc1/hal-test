...
async function setup() {
  try {
    ...
    try {
      execSync("pnpm --filter @halt-test/backend db:init", {
        stdio: "inherit",
        cwd: rootDir,
      });
    } catch (e) {
      console.error(
        "Failed to initialize database. Make sure dependencies are installed.",
      );
      process.exit(1);
    }
    ...
    try {
      ...
      console.warn(
        "Playwright installation failed or was partially successful. You might need to run it with sudo.",
      );
    }
