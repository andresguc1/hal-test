...
function getGitInfo() {
  try {
    ...
    const tag = execSync('git describe --tags --exact-match 2>/dev/null || echo ""', { cwd: ROOT_DIR, encoding: 'utf-8' }).trim();
    const isDirty = execSync('git status --porcelain', { cwd: ROOT_DIR, encoding: 'utf-8' }).trim() !== '';
    const commitDate = execSync('git log -1 --format=%cI', { cwd: ROOT_DIR, encoding: 'utf-8' }).trim();
    
    return { commit, shortCommit, branch, tag: tag || null, isDirty, commitDate };
  } catch (e) {
    return { commit: 'unknown', shortCommit: 'unknown', branch: 'unknown', tag: null, isDirty: false, commitDate: new Date().toISOString() };
  }
}

function getPackageVersions() {
  const rootPkg = JSON.parse(fs.readFileSync(path.join(ROOT_DIR, 'package.json'), 'utf-8'));
  const cliPkg = JSON.parse(fs.readFileSync(path.join(ROOT_DIR, 'apps', 'cli', 'package.json'), 'utf-8'));
  const frontendPkg = JSON.parse(fs.readFileSync(path.join(ROOT_DIR, 'apps', 'frontend', 'package.json'), 'utf-8'));
  const webPkg = JSON.parse(fs.readFileSync(path.join(ROOT_DIR, 'apps', 'web', 'package.json'), 'utf-8'));
  const backendPkg = JSON.parse(fs.readFileSync(path.join(ROOT_DIR, 'apps', 'backend', 'package.json'), 'utf-8'));
...
function injectIntoBackend(metadata) {
  ...
  if (fs.existsSync(backendPublicDir)) {
    ...
    fs.writeFileSync(outputPath, JSON.stringify(metadata, null, 2));
    console.log(`✅ Release metadata copied to backend/public for runtime access`);
  }
}
