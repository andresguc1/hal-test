...
class RollbackManager {
  constructor(options = {}) {
    ...
    this.renderClient = this.renderApiKey
      ? new RenderClient(this.renderApiKey)
      : null;
    this.productionServiceId =
      options.productionServiceId || process.env.RENDER_PRODUCTION_SERVICE_ID;
    this.stagingServiceId =
      options.stagingServiceId || process.env.RENDER_STAGING_SERVICE_ID;
  ...
  async evaluateHealth(results) {
    ...
    const criticalFailed = failed.filter(
      (r) => r.url.includes("/api/status") || r.url.includes("/app/"),
    );
  ...
  async triggerAppRollback() {
    ...
    console.log(
      `   Rolling back to deploy: ${previousDeploy.id} (${previousDeploy.createdAt})`,
    );

    if (!this.dryRun) {
      await this.renderClient.rollbackDeploy(
        this.productionServiceId,
        previousDeploy.id,
      );
  ...
  async runFullRollback(backupName) {
    ...
    if (backupName) {
      ...
      console.log(
        "   ⚠️  Database schema may be incompatible with rolled-back app",
      );
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  switch (command) {
    ...
    default:
      console.log("Usage:");
      console.log(
        "  node rollback.js check           - Run health check and auto-rollback if needed",
      );
      console.log(
        "  node rollback.js rollback <backup> - Full rollback (app + db)",
      );
      console.log(
        "  node rollback.js app-rollback    - Rollback app only via Render",
      );
      console.log(
        "  node rollback.js db-rollback <backup> - Restore database from backup",
      );
      console.log(
        "  node rollback.js health          - Check production health",
      );
