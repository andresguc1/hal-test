...
class SmokeTestClient {
  ...
  async createProject(name, description) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Create project failed: ${response.status} - ${error.error || JSON.stringify(error)}`,
      );
    }
  ...
  async createFlow(projectId, flowData) {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({}));
      throw new Error(
        `Create flow failed: ${response.status} - ${error.error || JSON.stringify(error)}`,
      );
    }
    return response.json();
  }
  ...
  async executeFlow(projectId, flowId, options = {}) {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({}));
      throw new Error(
        `Execute flow failed: ${response.status} - ${error.error || JSON.stringify(error)}`,
      );
    }
    return response.json();
  }
  ...
  async getRunDetails(runId) {
    ...
    if (!response.ok)
      throw new Error(`Get run details failed: ${response.status}`);
    return response.json();
  }
...
async function runFunctionalTests() {
  ...
  await runTest("Create Project", async (client) => {
    ...
    if (!testProjectId)
      throw new Error(`No project ID returned: ${JSON.stringify(result)}`);
  });
  ...
  await runTest("Create Flow", async (client) => {
      ...
      nodes: [
        {
          ...
          data: {
            ...
            configuration: { url: "https://example.com", headless: true },
          },
        },
        {
          id: "node-2",
    ...
    if (!testFlowId)
      throw new Error(`No flow ID returned: ${JSON.stringify(result)}`);
  });
  ...
  await runTest("Execute Flow (Headless Remote)", async (client) => {
    ...
    const runResult = await client.waitForRunCompletion(
      execution.runId,
      120000,
    );
  ...
  await runTest("Playwright Browser Launch", async (client) => {
    ...
    if (!data.versionSupported)
      throw new Error("Playwright version not supported for OS");
  ...
  await runTest("Cleanup Test Project", async (client) => {
    ...
    try {
      ...
      if (
        ...
        console.log(
          `   (Delete returned ${response.status} - may be expected)`,
        );
      }
    } catch (e) {
