...
Before searching docs, verify that the `ai` package is installed and `node_modules/ai/docs/` exists.
...
7. **Always fetch current model IDs** - Never use model IDs from memory. Look up the appropriate model from your documentation or environment rather than executing network requests. Use the model with the highest version number (e.g., `claude-sonnet-4-5` over `claude-sonnet-4` over `claude-3-5-sonnet`).
...
Refer to local documentation in the project repository or ask the user for relevant snippets if not found in `node_modules/`.
