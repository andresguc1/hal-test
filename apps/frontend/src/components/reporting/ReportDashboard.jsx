...
export default function ReportDashboard({ runId, onClose }) {
          ...
          if (runData.flow_snapshot) {
            ...
            const processedNodes = snapshot.nodes.map((n) => {
              ...
              return {
                ...
                data: {
                  ...n.data,
                  status: step ? step.status : NODE_STATES.DEFAULT,
                  error: step?.error,
    ...
    setNodes((prev) =>
      prev.map((node) => {
        ...
        return {
          ...
          data: {
            ...node.data,
            status: stepForThisNode
              ? stepForThisNode.status
              : NODE_STATES.DEFAULT,
            active: stepForThisNode?.node_id === currentStep?.node_id,
          },
        };
      }),
    );
  }, [currentStepIndex, run, setNodes, currentStep?.node_id]);
  ...
  }, [isPlaying, currentStepIndex, run, playbackSpeed]);
