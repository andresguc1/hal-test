    ...
    >
      <div className="flex items-center gap-2">
        <div 
          ...
          {...listeners}
        >
          <GripVertical size={14} className="text-slate-600 group-hover/field:text-indigo-400 transition-colors" />
        ...
        <button type="button" onClick={() => removeField(index)} className="p-1.5 text-slate-600 hover:text-rose-400 hover:bg-rose-500/10 rounded-md transition-colors shrink-0">
          <Trash2 size={12} />
        </button>
      </div>
    </div>
...
const FormFillEditor = React.memo(({
  ...
  const updateField = useCallback((index, key, val) => {
  ...
  const addField = useCallback(() => {
    ...
    onChange([...fields, { id, type: "text", selector: "", value: "", delay: 0 }]);
  }, [fields, onChange]);
  ...
  return (
    <div className="space-y-3 mt-2 mb-2">
      <div className="flex justify-between items-center">
        ...
        <div className="group relative">
          ...
          <div className="absolute right-0 bottom-full mb-2 w-56 p-3 bg-slate-900 border border-slate-700 rounded-lg text-[10px] text-slate-300 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 shadow-xl space-y-1">
            ...
            <p>Fields are filled sequentially. Variables like {"{{var}}"} will be resolved at runtime.</p>
          </div>
        </div>
      </div>
      ...
      <button type="button" onClick={addField} className="w-full flex items-center justify-center gap-1.5 py-2 px-3 border border-dashed border-indigo-500/30 rounded-xl text-[10px] font-bold text-indigo-400/80 uppercase tracking-wider hover:bg-indigo-500/10 hover:border-indigo-500/50 hover:text-indigo-400 transition-all">
