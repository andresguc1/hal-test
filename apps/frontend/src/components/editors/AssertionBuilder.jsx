...
const ASSERTION_TYPES = [
  {
    value: "existence",
    label: "Existence",
    description: "Verify element exists or not",
    icon: "🔍",
  },
  {
    value: "visibility",
    label: "Visibility",
    description: "Verify element is visible or hidden",
    icon: "👁️",
  },
  {
    value: "text",
  ...
  },
  {
    value: "attribute",
    label: "Attribute",
    description: "Verify HTML attribute value",
    icon: "🏷️",
  },
  {
    value: "value",
    label: "Input Value",
    description: "Verify form input/select value",
    icon: "📥",
  },
  {
    value: "state",
    label: "Element State",
    description: "Verify enabled/disabled/checked/etc.",
    icon: "⚙️",
  },
  {
    value: "css_property",
    label: "CSS Property",
    description: "Verify computed CSS property",
    icon: "🎨",
  },
  {
    value: "page",
    label: "Page (URL/Title)",
    description: "Verify page URL or title",
    icon: "🌐",
  },
];
...
const ASSERTION_TYPES_NO_EXPECTED_VALUE = ["existence", "visibility", "state"];
...
function AssertionRow({
  ...
  const typeSupportsStringOptions = [
    "text",
    "attribute",
    "value",
    "css_property",
  ...
  const typeNeedsExpectedValue =
    !ASSERTION_TYPES_NO_EXPECTED_VALUE.includes(type);
        ...
        <div className="flex-1 space-y-3 min-w-0">
          ...
          {(needsAttribute || needsCssProperty) && (
            <div className="flex gap-2">
              <VariableInput
                value={
                  assertion[needsAttribute ? "attribute" : "cssProperty"] || ""
                }
                type="text"
                ...
                onChange={(val) =>
                  onUpdate(
                    index,
                    needsAttribute ? "attribute" : "cssProperty",
                    val,
                  )
                }
                placeholder={t(
                  needsAttribute
                    ? "nodes.placeholders.attributeName"
                    : "nodes.placeholders.cssProperty",
                )}
                className="w-full sm:w-48 px-3 py-2 text-xs font-mono"
              />
              <span className="text-[10px] text-slate-500 self-center px-2">
                {needsAttribute
                  ? t("nodes.fields.attributeName")
                  : t("nodes.fields.cssProperty")}
              </span>
            </div>
          )}
          ...
          {assertion.type === "page" &&
            assertion.operator &&
            assertion.operator.startsWith("title_") && (
              <div className="text-[10px] text-indigo-400/80 flex items-center gap-1">
                <HelpCircle size={10} />
                <span>
                  {t(
                    "nodes.hints.page_title",
                    "Uses page.title() — no element selector needed",
                  )}
                </span>
              </div>
            )}
...
export function AssertionBuilder({
  ...
  const handleTypeChange = useCallback(
    (index, newType) => {
      const newOperators =
        OPERATORS_BY_TYPE[newType] || OPERATORS_BY_TYPE.existence;
      onChange(
        assertions.map((a, i) => {
          if (i !== index) return a;
  ...
  const handleMove = useCallback(
    (index, direction) => {
      ...
      [newAssertions[index], newAssertions[newIndex]] = [
        newAssertions[newIndex],
        newAssertions[index],
      ];
      ...
      {assertions.length === 0 ? (
        <div className="p-6 rounded-xl border-2 border-dashed border-slate-700/50 bg-slate-950/30 text-center">
          ...
          <p className="text-sm text-slate-500 mb-3">
            {t("nodes.placeholders.assertions")}
          </p>
          <button
            type="button"
            onClick={handleAdd}
