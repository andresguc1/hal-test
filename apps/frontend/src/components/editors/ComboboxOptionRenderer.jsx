...
const TYPE_BADGE = {
    combobox: { label: 'Combobox', cls: 'bg-violet-500/15 text-violet-400 border-violet-500/30' },
    'aria_option': { label: 'Option', cls: 'bg-violet-500/15 text-violet-400 border-violet-500/30' },
};
...
const ACTION_BADGE = {
    ...
    SELECT: { labelKey: 'nodes.config.action_select', cls: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30' },
};
...
export const ComboboxOptionRenderer = React.memo(({ detectedOptions, config, onChange, t }) => {
    ...
    // For combobox, only one can be selected
    const selectedOption = detectedOptions.find((o) => getAction(o) === 'SELECT');
        ...
        <div className="space-y-2">
            ...
            <div className="border border-slate-700/60 rounded-xl overflow-hidden max-h-48 overflow-y-auto custom-scrollbar divide-y divide-slate-800/60">
                {detectedOptions.map((opt, idx) => {
                    ...
                    const badge = TYPE_BADGE[opt.type] || TYPE_BADGE.combobox;
                    ...
                    return (
                        ...
                        >
                            ...
                            {/* Action selector - for combobox, only NO_CHANGE/SELECT, only one can be SELECT */}
                            <div className="shrink-0 flex items-center">
                                <div className="flex items-center gap-0.5">
                                    {[ACTION_NO_CHANGE, 'SELECT'].map((a) => {
                                        const active = action === a;
                                        const badge = ACTION_BADGE[a];
                                        const disabledForSelect = a === 'SELECT' && selectedOption && selectedOption.id !== opt.id;
                                        const disabled = opt.enabled === false || disabledForSelect;
                                        return (
                                            <button
                                                key={a}
                                                type="button"
...
ComboboxOptionRenderer.displayName = 'ComboboxOptionRenderer';
export default ComboboxOptionRenderer;