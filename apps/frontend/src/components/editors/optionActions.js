...
export function findBestMatch(configured, detectedOptions) {
    ...
    if (configuredLabel) {
        ...
        let match = detectedOptions.find((o) => normalizeString(o.label) === normLabel);
        if (match) return { matchedOption: match, meta: { label: match.label, value: match.value } };
    }
    ...
    if (configuredValue !== undefined && configuredValue !== null && configuredValue !== '') {
        ...
        let match = detectedOptions.find((o) => o.value !== undefined && normalizeString(o.value) === normValue);
        if (match) return { matchedOption: match, meta: { label: match.label, value: match.value } };
    }
