...
const OptionPickerEditor = React.memo(({
    ...
    const [groupType, setGroupType] = useState(() => inferGroupType(persistedAtMount));
    ...
    const persistedSnapshotRef = useRef(JSON.stringify(persistedDetectedOptions || []));
    useEffect(() => {
    ...
    const capturePageContent = useCallback(async () => {
        ...
        if (!browserId) {
            setNotice(t("nodes.select_option.no_browser", "No active browser session. Launch a browser first."));
            return;
        }
        setLoading(true);
        setNotice(null);
        try {
            const res = await api.post("/actions/select_option/page_content", { browserId });
            const data = res.data || {};
            if (data.html && onStaticHtmlChange) {
                onStaticHtmlChange(data.html);
                toast.success(t("nodes.select_option.page_captured", "Page content captured for static detection."));
            }
        } catch (err) {
            console.error("[OptionPickerEditor] Capture page error:", err);
            setNotice(err.message || t("nodes.select_option.capture_failed", "Failed to capture page content."));
        } finally {
            setLoading(false);
        }
    ...
    const scanOptions = useCallback(async () => {
        ...
        if (!selector) {
            setNotice(t("nodes.select_option.container_required", "Enter the container selector first."));
            return;
        }
        setLoading(true);
        setNotice(null);
    ...
    }, [containerSelector, staticHtml, config, getBrowserId, onChange, onDetectedOptionsChange, t]);
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    {getBrowserId() && !staticHtml && (
                        <button
                            ...
                            title={t("nodes.select_option.capture_page_title", "Capture current page HTML for static detection (native <select> only)")}
                        >
                    ...
                    <button
                        ...
                        title={t("nodes.select_option.scan_title", "Read the available options from the container.")}
                    >
